...
let takeAction = function(context){
	...
	var others = function(mess){
		...
		if (max['1'] > 70){
			GraphAPI.sendPlainMessage(recipientId, max['0'] +' يا '+ context.userData.first_name + ' عامل ايه؟').then(()=>{
					...
					if (max['1'] > 70){
						...
						if(max['1'] > 70){
								let thenkRes = ['حبيبي دا بس من زوقك يا غالي 😊','ربنا يخليك يا قلبي','والله انت اللي برنس وكلك زوق ','متكسفنيش بقي 😊😊 ','حبيب قلبي','دا كتير علي والله ','انت اللي محترم ']
		...
		if (context.current.notifiy === true && !context.current.logName) {
			...
			if(max['1'] > 70){
				context.current = {}
				GraphAPI.sendPlainMessage(recipientId,' 😨 تمام زي متحب.').then(()=>{
					offer();
				})
			}
		}
