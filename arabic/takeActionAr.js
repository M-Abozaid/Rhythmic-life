	...
	var others = function(mess){
		...
		if (max['1'] > 70){
			GraphAPI.sendPlainMessage(recipientId, max['0'] +' يا '+ context.userData.first_name + ' عامل ايه؟').then(()=>{
					...
					console.log('max ',max);
					if (max['1'] > 70){
						let howAreRes = ['الحمد لله وانت اخبارك؟','كويس انت عامل ايه يا '+ context.userData.first_name + 'يا عثل؟','انا بأفضل حال؟','جعان شوية يا '+ context.userData.first_name +' ' ,'انا بخير طول ما انت بخير يا '+ context.userData.first_name + ' يا جميل 😊😊 ','انا كويس شكرا علي السؤال','عاوز انام 😴 ']
						GraphAPI.sendPlainMessage(recipientId, generateRandom(howAreRes) +' 😀😀 ').then(()=>{
							context.current.main = 'howAreYou';
							resolve(context);
						})
					}else{
						let thank = ['شكرا ','الف شكر ','الله ينور ','شغل عالي','برافو','احسنت','بوت ممتاز','رائع','انت رائع','كويس','انت برنس']
						...
						if(max['1'] > 70){
							...
							let  insult = ['خول','يا خول ','يا متناك','ياع عرص','يا علق ','كسمك','متناك','عرص','اهبل ','هبيط','حمار','انت حمار','بقرة','حموسة','غبي','انت غبي','طيزك','طيزك حمرا','كس اختك','كلب']
							let max = fuzzChech(insult,context.msg);
							if(max['1'] > 70){
								let insultRes = ['عيب كده احترم نفسك','متحترم نفسك يا عم انت 😡😡 ','مينفعش كده خليك محترم 😡😡','الله يسمحك 😡😡','الشتيمة بتلف تلف وترجع لصاحبها 😡😡','متحترم نفسك يا جدع انت 😡😡 ','عيب كده اله','ملكش دعوة 😛😛 ']
								GraphAPI.sendPlainMessage(recipientId, generateRandom(insultRes)+' 😒😒').then(()=>{
								offer();
								})
							}else{
								let confusedRes = ['مش فاهم انت تقصد ايه, متجرب حاجة من دول','اكدب عليك لو قولتلك فهمت حرف من اللي انت قولته','انا قرات كل حاجة اني افهم كلمm. ','معلش اخوك فهمة علي قده شوية. 👶👶','ايه اللي خلاك تفكر في الحجات الوحشة دي. ','ودا من ايه لمؤخذة. ','فسر اكتر','ايه يا عم اللي انت بتقوله ده انا مفهمتش حاجة.','مليش في الكيما الصراحة','لا كده الموضوع بقي معقد قوي 😨😨.','طيب حاضر.','ماشي ','حصل (y)','من عنيا.','انت تؤمر']
